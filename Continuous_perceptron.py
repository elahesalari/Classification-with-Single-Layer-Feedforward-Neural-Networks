...
def keras_per(x_train,x_test,y_train,y_test):
    ...
    # categorical_crossentropy=multiclass log los , optimizer=update of weight

    model.fit(x_train,y_train,epochs=100,verbose=0 )
    resualt = model.evaluate(x_test,y_test,verbose=0)

    # print(f'test resualts - loss {resualt[0]} , Accuracy: {resualt[1]}')
    # exit()
    return resualt
...
def noisy(x,y,pr):
    ...
    for i in range(data.shape[0]):
        ...
        data[i,idx] = r
        # plt.imshow(data[i].reshape(60,60),'gray')
        # plt.show()
        # exit()
